...
public class Security {
    ...
    public static void createUser(String loginUser, String passwordUser, Login loginFrame){
        if(!Objects.equals(loginUser, "") && !Objects.equals(loginUser, "Login")) {
            if (!Security.loginExists(loginUser)) {
                try (Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/mydb", "root", "root")) {
...
// enter user (we add Login loginFrame to ensure closing of the Login frame in case everything is successful)
    public static void enterUser(String loginUser, String passwordUser, Login loginFrame) {
        if (!Objects.equals(loginUser, "")) {
            ...
            if (Security.loginExists(loginUser)) {
                byte[] salt = Security.getSaltByLogin(loginUser);
                String hashedPasswordUser = Security.getHash(passwordUser, salt);
                ...
                if (Objects.equals(hashedPasswordDB, hashedPasswordUser)) {

                    //if successful starts the main frame
                    loginFrame.dispose();
                    new GUI(loginUser);
                    Group.loadGroupsFromDatabase(loginUser, "group_id asc");

                    // check all the deadlines. If any of them is tomorrow, display warning.
                    SwingUtilities.invokeLater(() -> {
                        try (Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/mydb", "root", "root")){
                            PreparedStatement checkDeadlines = conn.prepareStatement("select count(*) from projects_" + loginUser + " where deadline = date_add(curdate(), interval 1 day)");
                            ...
                            if (countExpiringDeadlines.next() && countExpiringDeadlines.getInt(1) > 0) {
                                // if there are any expiring deadlines, add them to the list
                                List<String> expiringProjects = new ArrayList<>();
                                PreparedStatement fetchExpiringProjects = conn.prepareStatement("select project_name from projects_" + loginUser + " where deadline = date_add(curdate(), interval 1 day)");
                                ResultSet rs = fetchExpiringProjects.executeQuery();
                                while (rs.next()) {
                                    expiringProjects.add(rs.getString("project_name"));
                                }
                                // display the warning with names of the projects
                            ...
                            // now check already expired deadlines
                            PreparedStatement countExpiredDeadlines = conn.prepareStatement("select count(*) from projects_" + loginUser + " where deadline < curdate()");
                            ResultSet rs = countExpiredDeadlines.executeQuery();
                            if (rs.next() && rs.getInt(1) > 0){
                                // add expired projects names to the list
                                List<String> expiredProjects = new ArrayList<>();
                                PreparedStatement fetchExpiredProjects = conn.prepareStatement("select project_name from projects_" + loginUser + " where deadline < curdate()");
                                ResultSet rs1 = fetchExpiredProjects.executeQuery();
                                while (rs1.next()){
                                    expiredProjects.add(rs1.getString("project_name"));
                                }
                                // display the warning
                                ...
                                PreparedStatement deleteExpiredProjects = conn.prepareStatement("delete from projects_" + loginUser + " where deadline < curdate()");
                ...
                JOptionPane.showMessageDialog(null, "User with this login doesn't exist.");
            }
        } else {
            JOptionPane.showMessageDialog(null, "Login must not be null.");
        }

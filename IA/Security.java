...
public class Security {
    ...
    public static void createUser(String loginUser, String passwordUser, Login loginFrame){
        if(!Objects.equals(loginUser, "") && !Objects.equals(loginUser, "Login")) {
            if (!Objects.equals(passwordUser, "Password")){
                try (Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/mydb", "root", "root")) {
                    // check if user's database contains userauth table. We place it here, because userauth is used in loginExists method.
                    // We need to check userauth before its execution.
                    try (PreparedStatement checkUserauth = conn.prepareStatement("select * from information_schema.TABLES where TABLE_SCHEMA" +
                            " = 'mydb' and TABLE_NAME = 'userauth'")) {
                        ResultSet checker = checkUserauth.executeQuery();
                        // if no, create it
                        if (!checker.next()) {
                            PreparedStatement createUserauth = conn.prepareStatement("create table userauth (login varchar(100) primary key," +
                                    " password varchar(64), salt binary(16))");
                    ...
                    if (!loginExists(loginUser)) {
                        ...
                        if (affectedRows > 0) {
                            ...
                            String sql = "create table " + tableNameAcc + " (account_id int auto_increment" +
                                    " primary key, account_name varchar(255) not null, age int, email varchar(50), phone_number" +
                                    " varchar(50), workID varchar(50), changed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP not null)";
                            try (Statement st = conn.createStatement()) {
                                st.executeUpdate(sql);
                            }

                            String tableNameGr = "gr_" + loginUser;
                            String sqlGr = "create table " + tableNameGr + " (group_id int auto_increment primary key, name varchar(255) not null unique," +
                            ...
                            String sqlJun = "create table " + tableNameJun + " (group_id int, account_id int, primary key (group_id, account_id)," +
                                    " foreign key (group_id) references " + tableNameGr + "(group_id), foreign key (account_id) references "
                                    + tableNameAcc + "(account_id) on delete cascade);";
                            try (Statement st = conn.createStatement()) {
                                st.executeUpdate(sqlJun);
                            }
                            // create tables for files and projects and junction tables for them
                            try (Statement st = conn.createStatement()) {
                                st.executeUpdate("create table files_" +
                                        loginUser + " (file_id int auto_increment primary key, account_id int, file_name varchar(255) not null," +
                                        " file_path varchar(255) not null, file_type varchar(50), last_change timestamp default current_timestamp not null," +
                                        " file_creation timestamp not null, foreign key (account_id) references " + tableNameAcc + "(account_id) on delete cascade)");
                            }
                            try (Statement st = conn.createStatement()) {
                                st.executeUpdate("create table projects_" + loginUser + " (project_id int auto_increment primary key," +
                                        " project_name varchar(100) not null, description text, deadline date, changed_at timestamp default current_timestamp not null);");
                            }
                            try (Statement st = conn.createStatement()) {
                                st.executeUpdate("create table projects_junction_" + loginUser + " (account_id int, project_id int," +
                                        " primary key (account_id, project_id),foreign key (account_id) references " + tableNameAcc +
                                        "(account_id), foreign key (project_id) references projects_" + loginUser + "(project_id) on delete cascade)");
                            }
                ...
                JOptionPane.showMessageDialog(null, "Error. Password must be valid.");
            }
        } else {
        ...
        if (!Objects.equals(loginUser, "") && !Objects.equals(loginUser, "Login")){
            try (Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/mydb", "root", "root")){
                ...
                try (PreparedStatement checkUserauth = conn.prepareStatement("select * from information_schema.TABLES where TABLE_SCHEMA" +
                    ...
                    // if no, create it
                    if (!checker.next()) {
                        PreparedStatement createUserauth = conn.prepareStatement("create table userauth (login varchar(100) primary key, password varchar(64)," +
                                " salt binary(16))");
                        createUserauth.executeUpdate();
                    }
                }
                    ...
                    if (Objects.equals(hashedPasswordUser, hashedPasswordDB)) {
                        ...
                        SwingUtilities.invokeLater(() -> {
                            ...
                            try (Connection conn1 = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/mydb", "root", "root")) {
                                try (PreparedStatement checkDeadlines = conn1.prepareStatement("select count(*) from projects_" + loginUser +
                                    ...
                                    if (countExpiringDeadlines.next() && countExpiringDeadlines.getInt(1) > 0) {
                                        ...
                                        PreparedStatement fetchExpiringProjects = conn1.prepareStatement("select project_name from projects_" + loginUser +
                                                " where deadline = date_add(curdate(), interval 1 day)");
                                ...
                                try (PreparedStatement countExpiredDeadlines = conn1.prepareStatement("select count(*) from projects_" + loginUser
                                    ...
                                    if (rs1.next() && rs1.getInt(1) > 0) {
                                        ...
                                        PreparedStatement fetchExpiredProjects = conn1.prepareStatement("select project_name from projects_" + loginUser
                                                + " where deadline < curdate()");
                                        ...
                                        JLabel warningText = new JLabel("Warning! Some projects have been expired and were automatically deleted");
                                        warningText.setFont(new Font("Arial", Font.BOLD, 20));
                                        warningPanel.add(warningText, BorderLayout.NORTH);
                                        ...
                                        PreparedStatement deleteExpiredProjects = conn1.prepareStatement("delete from projects_" + loginUser
                                                + " where deadline < curdate()");
